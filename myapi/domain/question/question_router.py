...
"""
router 객체를 생성해 FastAPI 앱에 등록해야지만, 
FastAPI가 요청받은 URL을 해석해 결과를 리턴하는 
행위(라우팅) 기능이 동작한다.
"""
router = APIRouter(
    prefix="/api/question",
...
"""
db: Session 의미란, db 객체가 Session 타입임을 의미한다.
세션은 웹 컨테이너에서 클라이언트 정보를 서버에 보관한다.(쿠키는 웹 브라우저에 정보 저장)
